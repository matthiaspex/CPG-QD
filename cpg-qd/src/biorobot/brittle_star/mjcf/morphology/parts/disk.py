...
class MJCFBrittleStarDisk(MJCFMorphologyPart):
    ...
    def _build_pentagon(self) -> None:
        ...
        num_arms = self.morphology_specification.number_of_arms
        ...
        # box size
        arm_angle_delta = np.pi / num_arms
        box_y_size = radius * np.sin(arm_angle_delta)
        box_x_size = radius * np.cos(arm_angle_delta) / 2

        angles = np.linspace(0, 2 * np.pi, num_arms+1)[:-1]
        angles += angles[1] / 2 # rotate to align with arms
        ...
        for i, angle in enumerate(angles):
            ...
            self.mjcf_body.add(
                ...
                contype=0,     # does not have physical interactions, purely visual
                conaffinity=0, # does not have physical interactions, purely visual
                mass=0,        # does not have physical interactions, purely visual
            )
        ...
        self.mjcf_body.add(
            ...
            size=[radius, height],
            rgba=[0, 0, 0, 0],
            contype=1, # DOES HAVE PHYSICAL INTERACTIONS
            conaffinity=0,
    ...
    def _build_arm_connections(self) -> None:
        ...
        arm_angles = np.linspace(0, 2 * np.pi, num_arms+1)[:-1]
        ...
        if self.morphology_specification.actuation_specification.use_tendons.value:
            if num_arms != 5:
                raise NotImplementedError(
                    "Tendon attachment points are only implemented for 5-armed brittle stars."
                )
            self.distal_taps = []
